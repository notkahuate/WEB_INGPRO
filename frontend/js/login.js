...
function mostrarModalSeleccionEmpresa(empresas) {
    ...
    btnNo.onclick = () => {
        ...
        const paisSelect = document.getElementById("pais_company");
        if (paisSelect) paisSelect.disabled = false;
    };
    ...
    btnSi.onclick = () => {
        ...
        autocompletarYBloquear("company-name", data.Account_Name);
        autocompletarYBloquear("company-address", data.Billing_Street);
        autocompletarYBloquear("ciudad", data.Billing_City);
        autocompletarYBloquear("Estado_Provincia", data.Billing_State);
        autocompletarYBloquear("postal-code", data.C_digo_Postal_Facturaci_n);

        // 🌍 PAÍS (ESPECIAL)
        const paisSelect = document.getElementById("pais_company");

        if (data.Billing_Country && paisSelect) {
            ...
            if (encontrado) {
                ...
                paisSelect.style.backgroundColor = "#f5f5f5";

                // 🔥 CLAVE: avanzar manualmente
...
function autocompletarYBloquear(id, valor) {
    const input = document.getElementById(id);
    if (!input) return;
