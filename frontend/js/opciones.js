...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (e.key === "Enter") {
    ...
    if (texto.length > 0) {
      ...
      window.location.href = `/pages/busquedas.html?q=${encodeURIComponent(texto)}`;
    }
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔥 TITULO
  if (productTitle) {
    productTitle.textContent = selectedProduct.name || "Producto sin nombre";
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    productTitleEl.textContent = data.name || "Producto sin nombre";
...
const optionalSteps = Array.isArray(config.optional) ? config.optional : [];
    ...
    if (requiredSteps.length === 0 && optionalSteps.length === 0) {
      optionSectionsContainer.innerHTML = `
        <p class="no-config-message" style="
          ...
          margin-top: 20px;
        ">
          Este producto no cuenta con configuraciones.
        </p>
      `;
    ...
    const renderStepChrome = (step, index) => {
      ...
      const title = escapeHtml(step.title || `Paso ${index + 1}`);
    ...
    optionalTitle.textContent = "Opciones opcionales (no obligatorias)";
    ...
    optionalSteps.forEach((group, groupIndex) => {
      ...
      title.textContent = group.title || `Opcional ${groupIndex + 1}`;
      ...
      if (hasImages) {
        // Con imágenes: mismas filas visuales; un select oculto mantiene la lógica actual
  ...
  function showSection(index) {
    ...
    if (nextBtn) {
      nextBtn.textContent =
        index === optionSections.length - 1 ? "Finalizar requeridos" : "Siguiente ➡";
    }
  }
  ...
  document.addEventListener("change", (e) => {
    ...
    if (e.target.type === "radio") {
      ...
      // Si la opción tiene imagen, actualizar el resumen
    ...
    btn.addEventListener("click", (event) => {
     ...
     const selectedProduct = JSON.parse(localStorage.getItem("selectedProduct"));
        ...
        const newProduct = {
          ...
          name: productTitleEl.textContent.trim(),
