...
async function loadCatalogProducts() {
  ...
  for (const url of urls) {
    try {
      ...
      console.warn(`Could not load products from ${url}:`, error.message || error);
    }
  }
  return [];
}

// URL SEO: /product/{brand}/{model}/{category}
function buildProductUrl(p) {
  const seg = (v) => slugify(String(v || "")) || "";
  const brand = seg(p.brand || p.cf_marca || p.marca) || "brand";
  ...
  if (!modelRaw) {
    ...
    modelRaw = code && !isOpaque ? code : String(p.item_id || p.id || "model");
  }
  const model = seg(modelRaw) || "model";
  ...
  return `/product/${brand}/${model}/${category}`;
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 🔹 Load products from API (local first)
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!allProducts.length) {
    console.error("❌ Error loading recommended products");
    return;
  }
  ...
  randomProducts.forEach(p => {
    ...
    card.innerHTML = `
      ...
      <div class="card-info">
        ...
        <p>${p.cf_marca || p.brand || "Unknown brand"}</p>
      </div>
    `;
...
document.addEventListener("DOMContentLoaded", function () {
  // Mobile menu is handled by site-layout.js.
