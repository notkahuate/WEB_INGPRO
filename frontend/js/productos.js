...
function buildProductSlug(product) {
  ...
  const brand = slugify(product.brand || product.cf_marca || product.marca || 'marca') || 'marca';
  ...
  if (!modelRaw) {
    ...
    modelRaw = code && !isOpaque ? code : (product.item_id || product.id || 'modelo');
  }
  const model = slugify(modelRaw) || 'modelo';
  const rawCategory =
    product.categoria ||
...
// Marca → país (fuente de verdad: marcasPaises*.js), case-insensitive
...
// Alias EN/ES → id de país (para nombres de marcasPaises.js y marcasPaises_esp.js)
...
/** País de la marca según marcasPaises*.js (prioridad sobre brands[].country). */
...
// Alinear brands[].country con marcasPaises (evita que casi todo quede como 'de')
...
const _bucketByKey = (() => {
  ...
  for (const it of _categoryItemIndex) {
    ...
    // Solo código y etiqueta completa exacta (NO el nombre suelto: genera cruces ES/EN)
...
/** Índice primario: código de ítem (a011) → buckets. Independiente del idioma del título. */
const _bucketByCode = (() => {
  const map = new Map();
  for (const it of _categoryItemIndex) {
    if (!it.code) continue;
...
// Taxonomía precomputada (evita normalizeText × categorías en cada producto)
...
/** Match estricto (igualdad). Sin includes() para evitar cruces entre categorías. */
...
function extractItemCode(value) {
  ...
  // Formato nuevo: A002d / Z204hh (también admite legado A011)
...
/** Extrae todos los códigos tipo A002d / Z204hh / A011 de un valor. */
...
function collectProductItemCodes(p, categoryName, subcategoryName) {
  const codes = new Set();
  // En la BD el campo canónico es `categoria` (ej: "Presostatos - A011")
  ...
  // Si categoria no trae código, probar campos auxiliares
  if (!codes.size) {
    const fields = [
      p.cf_item,
      p.cf_codigo,
...
function adaptApiProduct(p, idx) {
  ...
  // Fuente de verdad: marcasPaises*.js → luego API → luego brands[]
  ...
  // ES: industria en cf_categoria
  const industryRaw = p.cf_categoria || '';
  const industryMatch =
  ...
  const industryName = industryMatch
    ? (typeof getIndustryLabel === 'function'
        ? getIndustryLabel(industryMatch, false)
        : industryMatch.es)
    : (industryRaw || '');

  // En BD: `categoria` = "Presostatos - A011" (ítem + código). Es la fuente de verdad.
  const categoriaRaw = String(p.categoria || '').trim();
  const categoryName = categoriaRaw;
  const subcategoryName = p.cf_subcategoria || p.cf_item || '';

  // Clasificación por CÓDIGO extraído de `categoria` (A011) — igual en ES y EN
  ...
  // Fallback estricto solo si categoria no trae código reconocible
  ...
  return {
    ...
    _searchBlob: [name, sku, brandName, industryName, industryRaw, categoriaRaw, p.cf_categoria || '', p.cf_subcategoria || '', p.cf_item || '', codesArr.join(' ')].join(' ').toLowerCase(),
...
function extractCategoryLetter(categoria, catIndex) {
  // Prefer letter from title: "Detection - Measurement (A)"
...
function subcategoryBlurb(subcategoriaSubtitulo, itemCount) {
  ...
  if (itemCount > 0) {
    return `Explora ${itemCount} tipos técnicos y soluciones especializadas en ${label}.`;
  }
  return `Soluciones industriales y equipamiento especializado en ${label}.`;
}

/* ---------- Fetch (cache local + adaptado por chunks) ---------- */
const PRODUCTS_CACHE_KEY = 'ingpro_catalog_v9';
const PRODUCTS_CACHE_TTL_MS = 5 * 60 * 1000; // alinea con Cache-Control del API
...
function writeProductsCache(list) {
  try {
    ...
    // sessionStorage lleno o bloqueado: se ignora
  }
}
  ...
  for (const url of API_URL_CANDIDATES) {
    try {
      ...
      if (url !== API_URL_CANDIDATES[0]) {
        console.info(`Productos cargados desde ${url}`);
      }
      return adaptProductsList(list);
    } catch (err) {
      lastErr = err;
      console.warn(`No se pudo cargar productos desde ${url}:`, err.message);
    }
  }
  console.error('❌ Error cargando productos:', lastErr);
  return [];
}
...
function setProductsLoadingUI(isLoading) {
  ...
  if (isLoading && !grid.dataset.hasProducts) {
    grid.innerHTML = '<div class="products-loading-hint" role="status">Cargando productos…</div>';
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Pintar hub/categorías al instante; no bloquear con el catálogo completo
  ...
  try {
    ...
    console.error('❌ Bootstrap de productos:', err);
    setProductsLoadingUI(false);
  ...
  const heroHtml = `
    <div class="taxonomy-category-view taxonomy-items-view">
      <div class="taxonomy-toolbar">
        ${renderTaxonomyBackButton(`Volver a ${categoriaPadre}`, `data-back-to-category="${encodeURIComponent(categoriaPadre)}"`)}
      </div>
      <article class="taxonomy-category-hero">
        ...
        <div class="taxonomy-category-hero-copy">
          ...
          <p>${items.length} tipos técnicos disponibles. Seleccione uno para consultar productos, marcas y documentación relacionada.</p>
          <div class="taxonomy-hero-meta">
            <span>${subCount} productos</span>
            <span>${subBrands.length} marcas</span>
            <span>${items.length} tipos</span>
          </div>
          <button type="button" class="category-view-all-btn subcat-hero-view-all" data-category="${escapeHtml(categoriaPadre)}" data-subcategory="${escapeHtml(subcategoriaName)}">Ver toda la subcategoría</button>
        </div>
      </article>
      ${renderCatalogNavHtml('categorias')}
      <div class="taxonomy-subcategories-head">
        <h4 class="taxonomy-subcategories-title">Tipos de ${escapeHtml(subcategoriaName)}</h4>
        <button type="button" class="taxonomy-view-all-subs subcat-hero-view-all" data-category="${escapeHtml(categoriaPadre)}" data-subcategory="${escapeHtml(subcategoriaName)}">Ver todos los productos →</button>
      </div>`;
    ...
    return `<article class="taxonomy-subcategory-card taxonomy-subcategory-card--catalog">
      <button class="taxonomy-subcategory-main subcat-item-row" type="button"
        ...
        <div class="taxonomy-subcategory-copy">
          ...
          <span class="taxonomy-expand-label">Ver productos →</span>
        </div>
...
function showProductsForSubcategory(subcategoriaName, categoriaPadre, searchTerm, itemCode = '') {
  ...
  // Navegación por ítem: el código manda (A011). El título ES/EN no filtra.
...
const SECTION_SWITCHES = [
  { id: 'categorias', title: 'Por Categoría', subtitle: 'Estructura técnica completa' },
  { id: 'marcas', title: 'Por Marca', subtitle: 'Fabricantes internacionales' },
  { id: 'paises', title: 'Origen del Producto', subtitle: 'Países de fabricación' },
  { id: 'industrias', title: 'Ver Todos', subtitle: 'Clasificación por industria' },
];
...
function renderCatalogNavHtml(activeSection = 'categorias') {
  return `<nav class="catalog-nav section-switcher" data-catalog-nav aria-label="Explorar catálogo">
...
function updateSectionBreadcrumb(sectionId) {
  ...
  if (cont) {
    const activeTab = sectionId === 'subcategorias' ? 'categorias' : (sectionId || 'inicio');
    cont.querySelectorAll('button[data-section]').forEach((b) =>
      b.classList.toggle('active', b.dataset.section === activeTab)
    );
  }
  syncCatalogNavActive(sectionId);
}
...
function initStats() {
  ...
  animateCountValue(document.getElementById('countryCount'), countries.length, ' países');
  ...
  animateCountValue(document.getElementById('industryCount'), industryTotal, ' industrias');
...
function renderIndustries() {
  ...
  const parts = INDUSTRIES.map((ind, index) => {
    ...
    return (
      ...
      `<small class="industry-card-count">${count} productos</small>` +
      `</span>` +
      `<span class="industry-card-cta">Ver productos →</span>` +
      `</button>`
    );
  });
...
function renderCountries() {
  ...
  for (const country of countries) {
    ...
    const txt = bs.length ? bs.slice(0, 5).join(', ') : 'Sin marcas registradas';
...
const _CATEGORY_DESCRIPTIONS = {
  'Detection - Measurement (A)': 'Sensores, transmisores, medidores de caudal, presion, nivel, temperatura, humedad, posicion, velocidad y medicion electrica para automatizacion industrial.',
  'METROLOGY - LABORATORY ( B )': 'Equipos de laboratorio, analisis fisicoquimico, metrologia, ensayos, monitoreo, inspeccion y componentes opticos de alta precision.',
  'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )': 'Automatismos, robotica industrial y de servicio, informatica industrial y software para Industria 4.0.',
  'Electricity - Electronics (D)': 'Produccion y distribucion de energia, alimentacion electrica, interruptores, reles, cables, conectores y componentes electronicos.',
  'Power Transmission - Mechanical components (E)': 'Rodamientos, guias lineales, transmision mecanica, actuadores y sistemas de posicionamiento industrial.',
  'Hydraulics - Pneumatics ( F)': 'Bombas, compresores, actuadores hidraulicos y neumaticos, valvulas, electrovalvulas, tuberias, racores y filtros.',
  'MATERIALS - TOOLS - COMPONENTS ( G)': 'Productos semimanufacturados, herramientas de corte, sujecion, montaje, componentes estandar y accesorios industriales.',
  'MACHINE-TOOLS (H)': 'Tornos, fresadoras, rectificadoras y centros de mecanizado de alta precision para la industria metal-mecanica.',
  'PRODUCTION MACHINES ( I )': 'Lineas de produccion, ensamble, envasado y maquinaria de manufactura.',
  'INDUSTRIAL MACHINES AND EQUIPMENT ( J )': 'Equipamiento industrial pesado y maquinaria de uso general para procesos productivos.',
  'FOOD INDUSTRY ( K )': 'Equipos para procesamiento, envasado y control de calidad en la industria de alimentos y bebidas.',
  'Logistics - Transport - Handling ( L )': 'Sistemas de transporte interno, montacargas, transportadores, almacenaje y manipulacion de cargas.',
  'Health - Safety - Environment ( M )': 'EPP, equipos de seguridad industrial, monitoreo ambiental y soluciones para salud ocupacional.',
  'Building - Construction ( N )': 'Materiales, equipos y herramientas para construccion, edificacion y obra civil.',
  'SMALL FARM EQUIPMENT ( O )': 'Maquinaria agricola, tractores, implementos y soluciones para agricultura industrial.',
  'SERVICES ( Z )': 'Servicios de calibracion, mantenimiento, asesoria tecnica e ingenieria industrial.'
};
...
/* Imagenes especificas por subcategoria. Key: "categoria||subcategoria".
   ...
   const _SUBCATEGORY_IMAGES = {
    'Detection - Measurement (A)||Medición de Caudal, Presión y Nivel': _imgCandidates('Medicion_de_cudal,presion-y-nivel.jpeg'),
    'Detection - Measurement (A)||Medición de la Temperatura y Humedad': _imgCandidates('Medicion_de_temperatura-y-humedad.jpeg'),
    'Detection - Measurement (A)||Medición de Posición, Velocidad y Aceleración': _imgCandidates('Medicion_de_posicion,velocidad-y-aceleracion.jpeg'),
    'Detection - Measurement (A)||Medición de Fuerza': _imgCandidates('Sensores_de_deteccion-y-proximidad.jpeg'),
    'Detection - Measurement (A)||Medición Acústica y Optica': _imgCandidates('Sensores_de_vision_y_opticos.jpeg'),
    'Detection - Measurement (A)||Medición Eléctrica': _imgCandidates('Medicion_electrica.jpeg'),
    'METROLOGY - LABORATORY ( B )||Equipo de Laboratorio': _imgCandidates('Laboratory_Equipment.png'),
    'METROLOGY - LABORATORY ( B )||Análisis Fisicoquímico': _imgCandidates('Analytical_Instrumentation.png'),
    'METROLOGY - LABORATORY ( B )||Metrologia y Ensayos': _imgCandidates('Metrology_and_Test_Equipment.png'),
    'METROLOGY - LABORATORY ( B )||Componentes Opticos': _imgCandidates('Optical_Components.png'),
    'METROLOGY - LABORATORY ( B )||Monitoreo e Inspección': _imgCandidates('_Inspection_and _Monitoring.png'),
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )||Automatismos': _imgCandidates('automatizacion.png'),
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )||Robótica Industrial': _imgCandidates('Roboticos_industriales.png'),
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )||Robótica de Servicio': _imgCandidates('Robots_servicios.png'),
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )||Informática industrial': _imgCandidates('industrial_computing.png'),
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )||Software industrial': _imgCandidates('intrudstrial_software.png'),
    'Electricity - Electronics (D)||Producción y Distribución de Energía': _imgCandidates('Producción_y_Distribución_de_Energía.png'),
    'Electricity - Electronics (D)||Alimentación Eléctrica': _imgCandidates('Alimentación_Eléctrica.png'),
    'Electricity - Electronics (D)||Interruptores y Relés': _imgCandidates('Interruptores_y_Relés.png'),
    'Electricity - Electronics (D)||Cables, Conectores y Cajas': _imgCandidates('Cables_Conectores_y_Cajas.png'),
    'Electricity - Electronics (D)||Componentes Electrónicos': _imgCandidates('Componentes_Electrónicos.jpeg'),
    'Power Transmission - Mechanical components (E)||Rodamientos, Guías Lineales': _imgCandidates('Rodamientos_Guías_Lineales.jpeg'),
    'Power Transmission - Mechanical components (E)||Transmisión Mecánica': _imgCandidates('Transmisión_Mecánica.jpeg'),
    'Power Transmission - Mechanical components (E)||Actuadores y Posicionamiento': _imgCandidates('Actuadores_y_posicionamiento.jpeg'),
    'SERVICES ( Z )||CERTIFICACIONES': _imgCandidates('CERTIFICACIONES.jpeg'),
    'SERVICES ( Z )||TRABAJO TÉCNICO/MANO DE OBRA': _imgCandidates('TRABAJO TÉCNICO/MANO_DE_OBRA.jpeg'),
    'SERVICES ( Z )||LOGÍSTICA': _imgCandidates('LOGISTICA.jpeg'),
    'SMALL FARM EQUIPMENT ( O )||Herramientas agrícolas pequeñas': _imgCandidates('Herramientas_agrícolas_pequeñas.jpeg'),
    'SMALL FARM EQUIPMENT ( O )||Conservación de las vías de circulación y de las zonas verdes': _imgCandidates('Conservación_verde.jpeg'),
    'Building - Construction ( N )||Acondicionamiento de Edificios Industriales': _imgCandidates('Acondicionamiento_de_Edificios_Industriales.jpeg'),
    'Building - Construction ( N )||Maquinarias y Equipos de Minas y Obras': _imgCandidates('Maquinarias_Obras.jpeg'),
    'Building - Construction ( N )||Herramientas para obra': _imgCandidates('Herramientas_obra.jpeg'),
    'Building - Construction ( N )||Producción de materiales de construcción': _imgCandidates('Producción_construcción.jpeg'),
    'Health - Safety - Environment ( M )||Tratamiento del Aire y Gestión del Ruido': _imgCandidates('SALUDSEGURIDADMEDIOAMBIENTE_0.jpeg'),
    'Health - Safety - Environment ( M )||Tratamiento del Agua': _imgCandidates('SALUDSEGURIDADMEDIOAMBIENTE_4.jpeg'),
    'Health - Safety - Environment ( M )||Tratamiento de Residuos': _imgCandidates('SALUDSEGURIDADMEDIOAMBIENTE_3.jpeg'),
    'Health - Safety - Environment ( M )||Equipos de Protección Individual': _imgCandidates('SALUDSEGURIDADMEDIOAMBIENTE_2.jpeg'),
    'Health - Safety - Environment ( M )||Seguridad para Máquinas y en Locales': _imgCandidates('SALUDSEGURIDADMEDIOAMBIENTE_1.jpeg'),
    'Health - Safety - Environment ( M )||Higiene y Limpieza': _imgCandidates('Higiene_Limpieza.jpeg'),
    'Logistics - Transport - Handling ( L )||Manutención y Elevación': _imgCandidates('LOGISTICATRANSPORTEMANIPULACION_0.jpeg'),
    'Logistics - Transport - Handling ( L )||Acondicionamiento y Embalaje': _imgCandidates('LOGISTICATRANSPORTEMANIPULACION_1.jpeg'),
    'Logistics - Transport - Handling ( L )||Transporte Ferroviario': _imgCandidates('LOGISTICATRANSPORTEMANIPULACION_3.jpeg'),
    'Logistics - Transport - Handling ( L )||Almacenamiento': _imgCandidates('LOGISTICATRANSPORTEMANIPULACION_4.jpeg'),
    'Logistics - Transport - Handling ( L )||Transportadores': _imgCandidates('LOGISTICATRANSPORTEMANIPULACION_2.jpeg'),
    'FOOD INDUSTRY ( K )||Procesamiento de la carne': _imgCandidates('INDUSTRIAALIMENTARIA_5.jpeg'),
    'FOOD INDUSTRY ( K )||Procesamiento del pescado': _imgCandidates('INDUSTRIAALIMENTARIA_4.jpeg'),
    'FOOD INDUSTRY ( K )||Procesamiento de frutas y hortalizas': _imgCandidates('INDUSTRIAALIMENTARIA_1.jpeg'),
    'FOOD INDUSTRY ( K )||Transformación de la leche': _imgCandidates('INDUSTRIAALIMENTARIA_3.jpeg'),
    'FOOD INDUSTRY ( K )||Panaderías-pastelerías': _imgCandidates('INDUSTRIAALIMENTARIA_0.jpeg'),
    'FOOD INDUSTRY ( K )||Transformación de cereales y producción de pasta': _imgCandidates('INDUSTRIAALIMENTARIA_2.jpeg'),
    'FOOD INDUSTRY ( K )||Otras Máquinas para la Industria Agroalimentaria': _imgCandidates('INDUSTRIAALIMENTARIA_5.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Equipos para el tratamiento de superficies': _imgCandidates('MAQUINASINDUSTRIALES_0.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Tratamiento de Superficies': _imgCandidates('MAQUINASINDUSTRIALES_1.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Hornos y Tratamiento Térmico': _imgCandidates('MAQUINASINDUSTRIALES_4.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Impresion, Marcado y Grabado': _imgCandidates('MAQUINASINDUSTRIALES_3.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Soldadura, Atornillamiento y Pegado': _imgCandidates('MAQUINASINDUSTRIALES_5.jpeg'),
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )||Mezclado y Dosificación': _imgCandidates('MAQUINASINDUSTRIALES_6.jpeg'),
    'Hydraulics - Pneumatics ( F)||Bombas': _imgCandidates('HIDRAULICANEUMATICA_0.jpeg'),
    'Hydraulics - Pneumatics ( F)||Compresores': _imgCandidates('HIDRAULICANEUMATICA_1.jpeg'),
    'Hydraulics - Pneumatics ( F)||Actuadores Hidráulicos y Neumáticos': _imgCandidates('HIDRAULICANEUMATICA_0.jpeg'),
    'Hydraulics - Pneumatics ( F)||Válvulas y Electroválvulas': _imgCandidates('HIDRAULICANEUMATICA_4.jpeg'),
    'Hydraulics - Pneumatics ( F)||Tuberías, Racores': _imgCandidates('HIDRAULICANEUMATICA_2.jpeg'),
    'Hydraulics - Pneumatics ( F)||Filtros y Separadores': _imgCandidates('HIDRAULICANEUMATICA_3.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Productos químicos industriales': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_6.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Productos Semimanufacturados': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_1.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Lubricación': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_3.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Otros equipamientos de carrocería': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_0.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Elementos Estándares Mecánicos': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_5.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Equipos de mantenimiento de vehículos': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_4.jpeg'),
    'MATERIALS - TOOLS - COMPONENTS ( G)||Herramientas manuales, herramientas eléctricas ( Utillaje )': _imgCandidates('MATERIALESHERRAMIENTASCOMPONENTES_2.jpeg'),
    'MACHINE-TOOLS (H)||Máquinas de Mecanizado': _imgCandidates('MÁQUINASHERRAMIENTA_0.jpeg'),
    'MACHINE-TOOLS (H)||Máquinas de Acabado': _imgCandidates('MÁQUINASHERRAMIENTA_3.jpeg'),
    'MACHINE-TOOLS (H)||Herramientas de Corte': _imgCandidates('MÁQUINASHERRAMIENTA_1.jpeg'),
    'MACHINE-TOOLS (H)||Equipos para Máquinas Herramientas': _imgCandidates('MÁQUINASHERRAMIENTA_2.jpeg'),
...
/** Lookup por título exacto o por letra de categoría: "(A)", "( B )", etc. */
...
function _subcategoryImageCandidates(catTitulo, subTitulo) {
  ...
  // Fallback: misma letra de categoría + mismo subtítulo
  ...
  function renderCategoryGrid() {
    ...
    const cards = categorias.map((categoria, catIndex) => {
      ...
      const catDesc = (typeof _CATEGORY_DESCRIPTIONS !== 'undefined' && _lookupByCatTitle(_CATEGORY_DESCRIPTIONS, categoria.titulo))
        || `Explora ${categoria.subcategorias.length} subcategorías y encuentra los productos que necesitas.`;
      ...
      return `<article class="taxonomy-subcategory-card taxonomy-subcategory-card--catalog" data-cat-index="${catIndex}">
        <button class="taxonomy-subcategory-main" type="button" data-category-open="${catIndex}" aria-label="Abrir subcategorías de ${escapeHtml(categoria.titulo)}">
          <div class="taxonomy-subcategory-copy">
            ...
            <div class="taxonomy-subcategory-stats">
              ...
              <span><strong>${brandCount}</strong> marcas</span>
              <span><strong>${categoria.subcategorias.length}</strong> subcategorías</span>
            </div>
            ${brandTagsHtml}
            <span class="taxonomy-expand-label">Explorar categoría →</span>
          </div>
    ...
    grid.innerHTML = `<div class="taxonomy-category-view taxonomy-category-view--root">
      <div class="taxonomy-subcategories-head">
        <h4 class="taxonomy-subcategories-title">Explore todas las categorías</h4>
      </div>
  ...
  function renderSubcategoriesFor(catIndex) {
    ...
    const cards = categoria.subcategorias.map((sub, subIndex) => {
      ...
      return `<article class="taxonomy-subcategory-card taxonomy-subcategory-card--catalog" data-cat-index="${catIndex}" data-sub-index="${subIndex}">
        <button class="taxonomy-subcategory-main" type="button" data-subcategory-open="${subIndex}" aria-label="Abrir ${escapeHtml(sub.subtitulo)}">
          <div class="taxonomy-subcategory-copy">
            <span class="taxonomy-subcategory-code taxonomy-subcategory-code--inline">${escapeHtml(code)}</span>
            <h3>${escapeHtml(sub.subtitulo)}</h3>
            <p>${escapeHtml(subcategoryBlurb(sub.subtitulo, itemCount))}</p>
            <div class="taxonomy-subcategory-stats">
              <span><strong>${subCount}</strong> productos</span>
              <span><strong>${subBrands.length}</strong> marcas</span>
              <span><strong>${itemCount}</strong> tipos</span>
            </div>
            ${subBrands.length ? `<div class="taxonomy-brand-tags">${brandTags}${extraBrands}</div>` : ''}
            <span class="taxonomy-expand-label">Abrir subcategoría →</span>
          </div>
          <div class="taxonomy-subcategory-media">${media}</div>
        </button>
      </article>`;
    ...
    grid.innerHTML = `
      <div class="taxonomy-category-view">
        <div class="taxonomy-toolbar">
          ${renderTaxonomyBackButton('Volver a categorías')}
        </div>
        <article class="taxonomy-category-hero">
          <div class="taxonomy-category-hero-media">${heroMedia}</div>
          <div class="taxonomy-category-hero-copy">
            <span class="taxonomy-eyebrow">Categoría activa</span>
            <h3>${escapeHtml(categoria.titulo)}</h3>
            <p>${escapeHtml(catDesc)}</p>
            <div class="taxonomy-hero-meta">
              <span>${productCount} productos</span>
              <span>${brandCount} marcas</span>
              <span>${categoria.subcategorias.length} subcategorías</span>
            </div>
            <button type="button" class="category-view-all-btn" data-cat-index="${catIndex}" data-view-all>Explorar productos</button>
          </div>
        </article>
        ${renderCatalogNavHtml('categorias')}
        <div class="taxonomy-subcategories-head">
          <h4 class="taxonomy-subcategories-title">Subcategorías de ${escapeHtml(categoria.titulo)}</h4>
          <button type="button" class="taxonomy-view-all-subs" data-cat-index="${catIndex}" data-view-all>Ver todas las subcategorías →</button>
        </div>
        <div class="taxonomy-subcategory-grid taxonomy-subcategory-grid--catalog">${cards}</div>
      </div>`;
  ...
  if (!categoryList.dataset.delegated) {
    categoryList.addEventListener('click', (e) => {
      ...
      if (itemBtn) {
        ...
        const codigoItem = (itemRaw.split(' - ')[1] || extractItemCode(itemRaw) || '').trim();
        showProductsForSubcategory(itemBtn.dataset.subcategory, itemBtn.dataset.category, '', codigoItem);
        return;
      }
...
function groupEntriesByLetter(entries) {
  ...
  for (const letter of Object.keys(groups)) {
    groups[letter].sort((a, b) => a.brand.name.localeCompare(b.brand.name, 'es'));
  }
  return groups;
}
...
function navigateBrandLetter(direction, filtered) {
  const { groups, available } = resolveBrandActiveLetter(filtered);
...
function getBrandEntries() {
  ...
  entries.sort((a, b) => a.brand.name.localeCompare(b.brand.name, 'es'));
  return entries;
}
...
function shortCategoryLabel(cat) {
  const map = {
    'Detección - Medición (A)': 'Detección y Medición',
    'Detection - Measurement (A)': 'Detección y Medición',
    'METROLOGÍA - LABORATORIO ( B )': 'Metrología',
    'METROLOGY - LABORATORY ( B )': 'Metrología',
    'ROBÓTICA - AUTOMATIZACIÓN - TI INDUSTRIAL ( C )': 'Robótica',
    'ROBOTICS - AUTOMATION - INDUSTRIAL IT ( C )': 'Robótica',
    'Electricidad - Electrónica (D)': 'Electricidad',
    'Electricity - Electronics (D)': 'Electricidad',
    'Transmisión de Potencia - Componentes mecánicos (E)': 'Transmisión',
    'Power Transmission - Mechanical components (E)': 'Transmisión',
    'Hidráulica - Neumática ( F)': 'Hidráulica',
    'Hydraulics - Pneumatics ( F)': 'Hidráulica',
    'MATERIALES - HERRAMIENTAS - COMPONENTES ( G)': 'Materiales',
    'MATERIALS - TOOLS - COMPONENTS ( G)': 'Materiales',
    'MÁQUINAS-HERRAMIENTA (H)': 'Máquinas herramienta',
    'MACHINE-TOOLS (H)': 'Máquinas herramienta',
    'MÁQUINAS DE PRODUCCIÓN ( I )': 'Producción',
    'PRODUCTION MACHINES ( I )': 'Producción',
    'MÁQUINAS Y EQUIPOS INDUSTRIALES ( J )': 'Maquinaria',
    'INDUSTRIAL MACHINES AND EQUIPMENT ( J )': 'Maquinaria',
    'INDUSTRIA ALIMENTARIA ( K )': 'Alimentos',
    'FOOD INDUSTRY ( K )': 'Alimentos',
    'Logística - Transporte - Manipulación ( L )': 'Logística',
    'Logistics - Transport - Handling ( L )': 'Logística',
    'Salud - Seguridad - Medio Ambiente ( M )': 'Seguridad',
    'Health - Safety - Environment ( M )': 'Seguridad',
    'Construcción - Edificación ( N )': 'Construcción',
    'Building - Construction ( N )': 'Construcción',
    'EQUIPOS PARA PEQUEÑAS EXPLOTACIONES AGRÍCOLAS ( O )': 'Agricultura',
    'SMALL FARM EQUIPMENT ( O )': 'Agricultura',
...
function brandDescription(entry) {
  ...
  const countryName = entry.country ? entry.country.name : 'origen internacional';
  if (cats) return `Fabricante de ${countryName} con presencia en ${cats.toLowerCase()}.`;
  return `Fabricante de ${countryName} presente en el catálogo industrial.`;
}
...
function renderBrandFeaturedCards(filtered, featuredThreshold) {
  ...
  if (!top.length) {
    grid.innerHTML = '<p class="bi-section-desc">No hay fabricantes que coincidan con los filtros.</p>';
    return;
  }
...
function renderBrandCardHtml(entry, featuredThreshold) {
  ...
  return `<article class="bi-featured-card" data-brand="${brand.id}" tabindex="0">` +
    ...
    `<div class="bi-card-metric"><strong>${count}</strong><span>Productos</span></div>` +
    `<div class="bi-card-metric"><strong>${families}</strong><span>Familias</span></div>` +
    `<div class="bi-card-metric"><strong>${categories.length || 1}</strong><span>Categorías</span></div>` +
    `</div>` +
    `<div class="bi-card-tags">${tags}</div>` +
    `<button class="bi-card-action" type="button" data-brand="${brand.id}">Explorar marca →</button>` +
    `</article>`;
}
...
function renderBrandResultsGrid(filtered, featuredThreshold, activeLetter, groups) {
  ...
  if (countEl) {
    countEl.textContent = activeLetter
      ? `${pageEntries.length} marcas con «${activeLetter}» · ${filtered.length} en total`
      : `${filtered.length} marcas encontradas`;
  }
  if (!grid) return;
  if (!filtered.length) {
    grid.innerHTML = '<p class="bi-section-desc">Sin resultados para los filtros seleccionados.</p>';
    return;
  }
  if (!pageEntries.length) {
    grid.innerHTML = '<p class="bi-section-desc">No hay marcas para esta letra con los filtros actuales.</p>';
    return;
  }
...
function renderBrandResultsTable(filtered, featuredThreshold, activeLetter, groups) {
  ...
  if (!body) return;
  if (!filtered.length) {
    body.innerHTML = '<tr><td colspan="5" style="padding:24px;text-align:center;color:#6b8cb8;">Sin resultados para los filtros seleccionados.</td></tr>';
    return;
  }
  if (!pageEntries.length) {
    body.innerHTML = '<tr><td colspan="5" style="padding:24px;text-align:center;color:#6b8cb8;">No hay marcas para esta letra con los filtros actuales.</td></tr>';
    return;
  }
...
function renderBrandLetterPagination(filtered, activeLetter, groups, available) {
  ...
  if (nav) {
    if (!filtered.length) {
      ...
      nav.innerHTML =
        `<button class="bi-letter-nav" type="button" data-letter-nav="prev" aria-label="Letra anterior" ${activeIdx > 0 ? '' : 'disabled'}>‹</button>` +
        `<div class="bi-letter-track" role="tablist" aria-label="Letras">` +
        ...
        `</div>` +
        `<button class="bi-letter-nav" type="button" data-letter-nav="next" aria-label="Letra siguiente" ${activeIdx >= 0 && activeIdx < available.length - 1 ? '' : 'disabled'}>›</button>`;
    }
  }
  ...
  if (meta) {
    if (!filtered.length) {
      ...
      meta.textContent = `Mostrando ${filtered.length} marca${filtered.length === 1 ? '' : 's'} · Seleccione una letra para filtrar`;
    } else {
      meta.textContent = `Mostrando ${pageCount} marca${pageCount === 1 ? '' : 's'} con la letra «${activeLetter}» · ${filtered.length} en total`;
    }
  }
}
...
function renderBrandAlphaIndex(filtered, activeLetter, groups) {
  ...
  nav.innerHTML = letters.map(letter => {
    const count = groups[letter]?.length || 0;
    const isActive = letter === activeLetter;
    return `<button class="bi-alpha-btn${isActive ? ' active' : ''}" type="button" data-letter="${letter}" title="${count ? count + ' marcas' : 'Sin marcas'}" ${count ? '' : 'disabled'}>${letter === '#' ? '#' : letter}</button>`;
  }).join('');
}
...
function renderSidebarFilters() {
  ...
  if (ind && typeof INDUSTRIES !== 'undefined') {
    ind.innerHTML = INDUSTRIES.map((industry) => {
      ...
      const label = typeof getIndustryLabel === 'function'
        ? getIndustryLabel(industry, false)
        : industry.es;
      const icon = typeof getIndustryIconClass === 'function'
...
function applyFilters() {
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    // 1) Ítem por CÓDIGO (A011) leído de BD.categoria — suficiente y fiable ES/EN
    if (itemCodeNorm) {
      if (!p._itemCodes || !p._itemCodes.has(itemCodeNorm)) continue;
    } else {
      // 2) Categoría / subcategoría por buckets derivados del código en `categoria`
      if (bucketKey) {
      ...
      // 3) Búsqueda libre (barra q=...), no navegación de catálogo
  ...
  if (activeFilters.industry && typeof INDUSTRIES !== 'undefined') {
    ...
    if (industry) {
      const label = typeof getIndustryLabel === 'function'
        ? getIndustryLabel(industry, false)
        : industry.es;
      filters.push({ type: 'industry', label: `Industria: ${label}` });
    }
  }
...
function updateProductsTitle() {
  ...
  if (activeFilters.brand) { const b = brands.find(x => x.id === activeFilters.brand); title = `Productos ${b ? b.name : ''}`; }
    ...
    const industry = INDUSTRIES.find((x) => x.id === activeFilters.industry);
    title = industry
      ? (typeof getIndustryLabel === 'function' ? getIndustryLabel(industry, false) : industry.es)
      : 'Industria';
  }
...
function _productCardHTML(product) {
  ...
  return `<a class="product-card" href="${escapeHtml(productUrl)}" data-pid="${escapeHtml(product.id)}">` +
    ...
    `<span class="product-btn">Ver producto</span></div></a>`;
}
...
function renderProducts() {
  ...
  // Guardar producto en localStorage antes de navegar (el href ya lleva a /producto/slug)
  if (!grid.dataset.delegated) {
    ...
    grid.addEventListener('click', (e) => {
      ...
      // Dejar que el <a href="/producto/..."> navegue normalmente
    });
